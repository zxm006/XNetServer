...
unsigned int get_disk_free( const std::string& strPath);
...
AFX_EXT_CLASS unsigned int GetTimeStamp(void);

/* 获取当前时戳(微秒microsecond) */
AFX_EXT_CLASS unsigned int GetMicroTimeStamp(void);

/* 获取当前时间(字符串) */
AFX_EXT_CLASS void getDate(const long utime_t,std::string &timenow);

/* 暂停(毫秒):将阻塞程序 */
AFX_EXT_CLASS void Pause(unsigned int ulMS);
...
AFX_EXT_CLASS std::string ULONG2STR(unsigned int value);
...
AFX_EXT_CLASS unsigned int STR2ULONG(const std::string& value);
...
AFX_EXT_CLASS std::string ULONG2HEX(unsigned int value);
AFX_EXT_CLASS unsigned int HEX2ULONG(const std::string& value);
...
//AFX_EXT_CLASS bool URLSplit(const std::string& url,std::string& protocol,std::string& serveraddr,unsigned short& serverport,std::string& filepath);
 ...
 // 获取文件大小
AFX_EXT_CLASS unsigned int GetFileSize(const std::string& strFileFullPath);
...
AFX_EXT_CLASS unsigned int GetBitMask(unsigned int nBit);
 ...
 * bEnabled:		权限值
 * <RETURN>:		设置权限后的新权限码 */
AFX_EXT_CLASS unsigned int SetPrivilege(unsigned int ulPrivilege,unsigned char ucBit,bool bEnabled);

/* 获得权限位
 * ulPrivilege:		权限码
 * ucBit:			权限位(取值范围1-20)
 * <RETURN>:		权限值 */
AFX_EXT_CLASS bool GetPrivilege(unsigned int ulPrivilege,unsigned char ucBit);
