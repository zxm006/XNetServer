...
//---------------------------------------------------------------------------------------
//获得高精度绝对时间
unsigned int GetMicroTimeStamp(void)
{
#ifdef WIN32
...
//---------------------------------------------------------------------------------------
unsigned int GetFileSize(const std::string& strFileFullPath)
{
	struct stat st;
...
//---------------------------------------------------------------------------------------
unsigned int GetBitMask(unsigned int nBit)
{
	unsigned int ulBitMask=0;
...
//---------------------------------------------------------------------------------------
unsigned int SetPrivilege(unsigned int ulPrivilege,unsigned char ucBit,bool bEnabled)
{
	unsigned int ulMask = 1 << (ucBit-1);
	if(bEnabled)
	{
	...
	{
		unsigned int ulMaskEx = ~ulMask;
		return ulPrivilege & ulMaskEx;
	}
}

//---------------------------------------------------------------------------------------
bool GetPrivilege(unsigned int ulPrivilege,unsigned char ucBit)
{
	unsigned int ulMask = 1 << (ucBit-1);
	unsigned int a = ulPrivilege & ulMask;
	if(a!=0)
	{
		return true;
	}
